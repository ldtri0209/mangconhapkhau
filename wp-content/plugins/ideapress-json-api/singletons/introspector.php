...
class JSON_API_Introspector {
  ...
  public function get_posts($query = false, $wp_posts = false, $shorten=null) {
    global $post;
    ...
    while (have_posts()) {
      ...
      if ($wp_posts) {
        $output[] = $post;
          ...
          if(!empty($shorten)){
              $output[] = new JSON_API_Post($post,true);
          }else{
  ...
  public function get_current_category() {
    global $ip_json_api;
    extract($ip_json_api->query->get(array('id', 'slug', 'category_id', 'category_slug')));
    if ($id || $category_id) {
      if (!$id) {
        $id = $category_id;
      }
  ...
  public function get_current_tag() {
    global $ip_json_api;
    extract($ip_json_api->query->get(array('id', 'slug', 'tag_id', 'tag_slug')));
    if ($id || $tag_id) {
      if (!$id) {
        $id = $tag_id;
      }
  ...
  public function get_current_author() {
    global $ip_json_api;
    extract($ip_json_api->query->get(array('id', 'slug', 'author_id', 'author_slug')));
    if ($id || $author_id) {
      if (!$id) {
        $id = $author_id;
      }
  ...
  protected function set_posts_query($query = false) {
    ...
    if ($ip_json_api->query->page) {
      $query['paged'] = $ip_json_api->query->page;
    }
    
    if ($ip_json_api->query->count) {
      $query['posts_per_page'] = $ip_json_api->query->count;
    }
    
    if ($ip_json_api->query->post_type) {
      $query['post_type'] = $ip_json_api->query->post_type;
    }
