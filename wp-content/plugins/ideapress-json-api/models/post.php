...
class JSON_API_Post {
  ...
  function JSON_API_Post($wp_post = null, $shorten = null) {
    if (!empty($wp_post)) {
        if(!empty($shorten)){
            $this->import_wp_object($wp_post, $shorten);
        }
        else{
  ...
  function save($values = null) {
    ...
    if (!empty($values['author'])) {
      $author = $ip_json_api->introspector->get_author_by_login($values['author']);
    ...
    if (isset($values['categories'])) {
      ...
      foreach ($categories as $category_slug) {
        ...
        $category = $ip_json_api->introspector->get_category_by_slug($category_slug);
  ...
  function import_wp_object($wp_post,$shorten=null) {
    global $ip_json_api, $post;
    $date_format = $ip_json_api->query->date_format;
    ...
    if(!empty($shorten))
    {
        $this->set_value('content', get_the_excerpt());
    }
    else{
  ...
  function set_value($key, $value) {
    global $ip_json_api;
    if ($ip_json_api->include_value($key)) {
      $this->$key = $value;
    } else {
      unset($this->$key);
    }
  ...
  function set_content_value() {
    global $ip_json_api;
    if ($ip_json_api->include_value('content')) {
      $content = get_the_content($ip_json_api->query->read_more);
  ...
  function set_categories_value() {
    global $ip_json_api;
    if ($ip_json_api->include_value('categories')) {
  ...
  function set_tags_value() {
    global $ip_json_api;
    if ($ip_json_api->include_value('tags')) {
      $this->tags = array();
  ...
  function set_author_value($author_id) {
    global $ip_json_api;
    if ($ip_json_api->include_value('author')) {
  ...
  function set_comments_value() {
    global $ip_json_api;
    if ($ip_json_api->include_value('comments')) {
      $this->comments = $ip_json_api->introspector->get_comments($this->id);
    } else {
      unset($this->comments);
    }
  }
  ...
  function set_attachments_value() {
    global $ip_json_api;
    if ($ip_json_api->include_value('attachments')) {
      $this->attachments = $ip_json_api->introspector->get_attachments($this->id);
    } else {
  ...
  function set_thumbnail_value() {
    global $ip_json_api;
    if (!$ip_json_api->include_value('thumbnail') ||
  ...
  function set_custom_fields_value() {
    global $ip_json_api;
    if ($ip_json_api->include_value('custom_fields') &&
        $ip_json_api->query->custom_fields) {
      $keys = explode(',', $ip_json_api->query->custom_fields);
  ...
  function get_thumbnail_size() {
    global $ip_json_api;
    if ($ip_json_api->query->thumbnail_size) {
      return $ip_json_api->query->thumbnail_size;
