...
class JSON_API_Comment {
  ...
  function comment_id_not_found() {
    global $ip_json_api;
    $ip_json_api->error("Post ID '{$_REQUEST['post_id']}' not found.");
  }
  
  function comment_closed() {
    global $ip_json_api;
    $ip_json_api->error("Post is closed for comments.");
  }
  
  function comment_on_draft() {
    global $ip_json_api;
    $ip_json_api->error("You cannot comment on unpublished posts.");
  }
  
  function comment_post_redirect() {
    global $comment, $ip_json_api;
    $status = ($comment->comment_approved) ? 'ok' : 'pending';
    $new_comment = new JSON_API_Comment($comment);
    $ip_json_api->response->respond($new_comment, $status);
  }
